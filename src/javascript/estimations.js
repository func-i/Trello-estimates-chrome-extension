...
(function() {
  var cardDetailsIsOpen, loadCode;
  ...
  loadCode = function() {
    ...
    cardInProgress = function(total_worked) {
      return $("#estimation_progress").css("width", "" + total_worked + "%");
    };
    ...
    bindGeneralEstimationEvents = function() {
      return $("#estimation_details").on("click", function() {
        var $estimationsSection;

        $estimationsSection = $(".estimations");
        if ($estimationsSection.css("display") === "none") {
          $estimationsSection.show();
          return $("#estimation_details").text("Hide Details");
        } else {
          $estimationsSection.hide();
          return $("#estimation_details").text("More Details");
        }
      });
    };
    populateEstimationSection = function() {
      return $.ajax("http://localhost:3000/estimations", {
        ...
        success: function(response) {
          ...
          if (response.estimations.length > 0) {
            total_estimation = total_estimation / (response.estimations.length * 1.0);
          }
          ...
          return bindGeneralEstimationEvents();
        }
      });
    };
